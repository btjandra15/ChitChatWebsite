...
<<<<<<< Updated upstream:Phase II/client/src/Pages/Home/components/Post/Post.js
...
>>>>>>> Stashed changes:Phase II/client/src/components/CreatePost/CreatePost.js
    ...
    if (!warning) {
      ...
      axios(configuration)
        .then((res) => {
          ...
          if (res.data && res.data.tabooWords && res.data.tabooWords.length > 0) {
            const tabooWordsMessage = `Post contains taboo words: ${res.data.tabooWords.join(', ')}`;
            alert(tabooWordsMessage);
          } else {
            alert("Successfully made a post");
          }
        })
